    ...
    try:
        ...
        with st.sidebar:
            ...
            if len(st.session_state.drive_folder_history) > 1:                
                if st.button("⬅️**Back to Previous Path**", disabled=st.session_state.folder_nav_in_progress, help ="Click to go back to the previous folder"):
                    ...
                    st.rerun()            
            # Display current full folder path built from folder history
            path_parts = []            
            for folder_id in st.session_state.drive_folder_history:
                path_parts.append(st.session_state.drive_folder_names.get(folder_id, 'Unknown Folder'))
            st.session_state.current_folder_path = ' > '.join(path_parts)
            st.info(f"**Current Path**:\n{st.session_state.current_folder_path}") 
            if not files:
            ...
            if st.session_state.selected_files:   
                ...
                if "delete_confirm_needed" not in st.session_state:
